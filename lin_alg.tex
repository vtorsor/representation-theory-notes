...
A \emph{tensor product} $V_1 \otimes_k V_2 \otimes \cdots \otimes_k V_n$
is a $k$-vector space along with a multilinear map
\[
\pi : V_1 \times \cdots \times V_n
\to  V_1 \otimes_k \cdots \otimes_k V_n
\]
such that for any multilinear map $\phi : V_1 \times \cdots \times V_n
\to W$ there exists a unique linear map
$\psi : V_1 \otimes_k \cdots \otimes_k V_n \to W$ such that
$\phi = \psi \circ \pi$.
\end{definition}
...
Given $(v_1,\ldots, v_n) \in V_1 \times \cdots \times V_n$,
let $v_1 \otimes v_2 \otimes \cdots \otimes v_n$ denote the image
$\pi(v_1,\ldots,v_n)$ in $V_1 \otimes_k \cdots \otimes_k V_n$.
...
$V_1 \times \cdots \times V_n$.  Let $[x]$ be the basis element
corresponding to $x$.
Let $I$ be the subspace of $\Omega$ spanned by
\[
[(v_1, \ldots, (\lambda v_i + v'_i), \ldots, v_n)]
-
\lambda[(v_1, \ldots,v_i, \ldots, v_n)] -
[(v_1, \ldots, v'_i, \ldots, v_n)]
\]
...
$\prod_{i=1}^n V_i \to \Omega$ and the quotient $\Omega \to
...
\begin{corollary}
The vector space $V_1 \otimes_k \cdots \otimes_k V_n$
...
v_1 \otimes \cdots \otimes v'_i \otimes \cdots \otimes v_n
\end{align*}
holds
for every index $i$, every $\lambda \in k$ and all $v_j,v'_j \in V_j$.
\end{corollary}
...
\begin{proposition}
If $B_1,\ldots B_n$ are bases for $V_1,\ldots,V_n$, then
\[
\{ b_1 \otimes \cdots \otimes b_n \mid b_1 \in B_1, \ldots, b_n \in B_n
\}
\]
is a basis for $V_1 \otimes_k \cdots \otimes_k V_n$.
\end{proposition}
...
\[
\phi(g) = \sum_{i=1}^n v^\vee_i \otimes g(v_i) \ .
\]
...
\[
T\left( \sum_{i=1}^n f_i \otimes v_i\right)
= \sum_{i=1}^n f_i(v_i)
\]
...
$n$, we define $\mathcal{T}^n(V)=\bigotimes_{i=1}^n V$ and
$\mathcal{T}^0(V)=k$.  We define the \emph{tensor $k$-algebra on $V$}
as the graded $k$-algebra
\[
\mathcal{T}(V) := \bigoplus_{n \ge 0} \mathcal{T}^n(V)
\]
with multiplication
\[\mathcal{T}^n(V) \times \mathcal{T}^m(V)
\to \mathcal{T}^{n+m}(V)
\]
defined via
\[
(v_1 \otimes \cdots \otimes v_n) (w_1 \otimes \cdots \otimes w_m)
...
\]
The graded piece $\mathcal{S}^n(V)$ is called the
\emph{$n$th symmetric power}.
\end{definition}
...
The composition
\[
\pi : V^n \to \mathcal{T}^n(V) \to \mathcal{S}^n(V)
\]
is a symmetric multilinear map such that for any
symmetric multilinear map
$\phi : V^n \to W$ there exists a unique linear map
$\psi : \mathcal{S}^n(V) \to W$
such that
$\phi = \psi \circ \pi$.
\end{proposition}
...
In particular, $\dim_k\ \mathcal{S}^d(k^n)=\binom{n+k-1}{k}$.
\end{proposition}
...
The image of a simple tensor $v_1 \otimes v_2 \otimes \cdots \otimes v_n$
is written $v_1 \wedge v_2 \wedge \cdots \wedge v_n$.
...
\begin{proposition}
If $v_1,\ldots,v_n \in V$ and $\sigma \in S_n$,
then
\[v_1 \wedge \cdots \wedge v_n =
\operatorname{sgn}(\sigma)
v_{\sigma(n)} \wedge \cdots \wedge v_{\sigma(n)}.
\]
\end{proposition}
...
\begin{proposition}
If $f \in \Lambda^p(V)$ and $g \in \Lambda^q(V)$,
then $f \wedge g = (-1)^{pq} g \wedge f$.
\end{proposition}
...
the permutation
\[
(1,\ldots,p+q) \mapsto (p+1,\ldots,p+q,1,\ldots,p)
\]
has sign $(-1)^{pq}$.
\end{proof}
...
In particular, $\dim_k\ \Lambda^d(k^n)=\binom{n}{k}$.
\end{proposition}
