...
for $e_0$ and $h_0$ if one writes the definitions in the ``right'' way.
There are good reasons to instead define $p_0=n$, but this does not
extend to the ring of symmetric functions discussed in the next section. 
\end{remark}
...
Let $\mathbb{Z}[[\{x_n\}_{n \in \mathbb{N}}]]$ be the ring of formal
...
$\mathbb{Z}[[\{x_n\}_{n \in \mathbb{N}}]]_{(d)}$ consists of those
