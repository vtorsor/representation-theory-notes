...
projection $\pi$, we obtain a direct sum decomposition
\[
k^n = \ker(\pi) \oplus \operatorname{im}(\pi) \ .
\]
Observe that idempotent $1-e$ is orthogonal to $e$ and its corresponding
projection $\pi'$ swaps the roles of the kernel and the image of $\pi$.
...
\]
where $V_i = \operatorname{im}(e_i)$ for $1 \le i \le r$.
