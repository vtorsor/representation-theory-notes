...
such that $gx=yh$.  We define
\[
\rho_g^{y,x}=\phi_y \circ \sigma_h \circ \phi_x^{-1}
\]
and set all other components to $0$.
...
map $\psi : V \to \operatorname{Ind}^G_H \to V$ such that
$\pi \circ \psi = \varphi$.
...
\section{Permutation and Monomial Representations}
...
Thus, summing over $G$ instead of $X$, we obtain the desired formula
after dividing by the size of the cosets $H$.
\end{proof}
...
but only a ring homomorphism when $G=H$.
...
\begin{proof}
Since $\psi(a^{-1}ga)=\psi(g)$, we have
\[
...
Note that all of the above facts for $C(G)$ hold for $R(G)$ as well.
...
by composition as $\varphi = \psi \circ \iota$.
...
transformation on Hom-sets so we have the desired isomorphism
of vector spaces.
\end{proof}
...
\begin{align*}
&(\operatorname{Ind}_H^G \varphi, \psi)_G\\
=& \dim_k\ \operatorname{Hom}^G_k(\operatorname{Ind}_H^G W, V)\\
=& \dim_k\ \operatorname{Hom}^H_k(W,\operatorname{Res}_H^G  V)\\
=&(\varphi, \operatorname{Res}_H^G \psi)_H .
\end{align*}
